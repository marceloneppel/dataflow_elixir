defmodule Dataflow.DirectRunner.PipelineSupervisor do
  use Supervisor
  alias Dataflow.DirectRunner.{TransformExecutor, ValueStore, StatsCollector}
  ...
  def init({leaf_transforms, values}) do
    ...
    # handle collecting stats
    stats = worker(StatsCollector, ["../log_ex_dataflow"])
    ...
    supervise([store, transform_registry, stats | workers], strategy: :one_for_one) #TODO: consider the correct strategy here
  end
end
