defmodule Dataflow.DirectRunner.ReducingEvaluator.PaneInfoTracker do
  ...
  @spec get_next(window, previous_pane, input_watermark, output_watermark, last?) :: PaneInfo.t
  ...
  def get_next(window, previous_pane, liwm, lowm, last?) do
    ...
    late_for_output? = Time.before?(max_win_ts, lowm)
    ...
    early_for_input? = Time.before_eq?(liwm, max_win_ts)
    ...
    %PaneInfo{first?: first?, last?: last?, timing: timing, index: index, non_speculative_index: non_spec_index}
  end
